        ...
        def _post(request, *args, **kwargs):
            ...
            try:
                ...
                if form.is_valid():
                    ...
                    requester_profile = accounts_models.Profile.objects.filter(username=request.user)
                    requested_profile = ContentType.objects.get_for_id(self.kwargs.get('content_type_id'))
                    requested_profile_username =requested_profile.get_object_for_this_type(pk=self.kwargs.get('object_id'))
                    requested_profile_user = User.objects.get(username=requested_profile_username)
                    requested_profile = accounts_models.Profile.objects.filter(username=requested_profile_user)
                    requester_profile.update(profilescore=F('profilescore')+.01)
                    requested_profile.update(profilescore=F('profilescore')+.1)
                    requester_profile.update(leaguescore=F('profilescore')+.01)
                    requested_profile.update(leaguescore=F('profilescore')+.1)
                    requester_profile.update(dailyleaguescore=F('profilescore')+.01)
                    requested_profile.update(dailyleaguescore=F('profilescore')+.1)
